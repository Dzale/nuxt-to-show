...
/*    $(".scroll-init  ul ").singlePageNav({
    ...
    $("body").on("click", ".airportNav", function (e) {
        e.preventDefault();
        $(".airportNav").removeClass('act-scrlink');
        $(this).addClass('act-scrlink');
        var to = "#" + this.id.replace("-scroll", "");
        $('html, body').animate({
            scrollTop: $(to).offset().top - 143
        }, 500);
        return false;
    });

    function overElement(id, postion, down = true){//na dole  je dobro
        let top = ($('#' + id).offset().top) - ( down ? 183 : 223);
        let bottom = top + $('#' + id).outerHeight(true) ;
        //console.log("RANGE FOR: " + id, top + " : " + bottom, postion );
    ...
    $(window).on("scroll", function (a) {
        ...
        $.each( $("[id$='-scroll']"), function( index, item ) {
            let el = item.id.replace("-scroll", "");
            if( overElement(el, scrollTop, prevPostion < scrollTop) ){
                $(".airportNav").removeClass('act-scrlink');
                $("#"+item.id).addClass('act-scrlink');
            }
        });
        //console.log(prevPostion < scrollTop ? 'DOWN' : "UP");
        prevPostion = scrollTop;
